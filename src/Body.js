    ...
    <React.Fragment>
      <div className="modal__container">
        <Modal
          ...
          send={onSendHandler}
          title={'Are you sure you would like to send the message'}
        >
          {inputValue
            ? `${inputValue} using the ${type} template`
            : 'Please enter a title'}
        </Modal>
      </div>
      <div className={`body ${props.next ? 'next' : ''}`}>
        <div className="body__container">
          <h2 className="title">Choose a template</h2>
          <select
            name="selection"
            className="body__selection"
          ...
          <button className="button modal-send" onClick={sendMessageHandler}>
            SEND
          </button>
        </div>
      </div>
